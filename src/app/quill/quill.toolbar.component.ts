...
export class QuillToolbarComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.highlightTagService.highLightTag.subscribe((tags) => {
      this.log.debug(`highlight tag service called`);
      this.log.debug(
        `about to toggle the following: ids -> ${tags.ids.toString()} type -> ${
          tags.type
        } state -> ${tags.active}`
      );
  ...
  highlightTag(ids: number[], type: string, active: boolean) {
    ...
    this.log.debug(
      `about to toggle the following: ids -> ${ids.toString()} type -> ${type} state -> ${active}`
    );
    for (let id of ids) {
      this.log.debug(
        `first button with id ${id} and type ${type} and this will be an active=${active} highlight`
      );
